...
gen = importlib.import_module('track-generation-grid-map')
...
class Piece(pygame.Surface):
    ...
    def __init__(self, typ: str, size: int = 50):
        ...
        if typ == 'b':
            self.surf = pygame.image.load(os.path.join(self.imagepath, 'building.png')).convert_alpha()
        elif typ == 'g':
            ...
            self.surf = pygame.image.load(os.path.join(self.imagepath, 't-cross.png')).convert_alpha()
        elif typ == 'x':
            self.surf = pygame.image.load(os.path.join(self.imagepath, '4-cross.png')).convert_alpha()
        elif typ == 'e':
            self.surf = pygame.image.load(os.path.join(self.imagepath, 'end.png')).convert_alpha()
...
def define_typ_rotation(arr):
    ...
    if len(trues) == 1:
        ...
        if 0 in trues and 1 in trues:
            typ = 's'
            rotation = 0
        elif 2 in trues and 3 in trues:
            typ = 's'
            rotation = 90
        elif 1 in trues and 2 in trues:
            typ = 'l'
            rotation = 0
        elif 0 in trues and 2 in trues:
            typ = 'l'
            rotation = 90
        elif 0 in trues and 3 in trues:
            typ = 'l'
            rotation = 180
        elif 1 in trues and 3 in trues:
            typ = 'l'
            rotation = 270
    elif len(trues) == 3:
        if 0 in trues and 2 in trues and 3 in trues:
            typ = 't'
            rotation = 0
        elif 0 in trues and 1 in trues and 3 in trues:
            typ = 't'
            rotation = 90
        elif 1 in trues and 2 in trues and 3 in trues:
            typ = 't'
            rotation = 180
        elif 0 in trues and 1 in trues and 2 in trues:
            typ = 't'
            rotation = 270
    else:
        typ = 'x'
...
def draw_track(sol, grid, piecesize):
    ...
    col = []
    for ij in np.ndindex(sol['STREET'].shape[:2]):
        if grid['STREET'][ij] == 1:
            typ, rotation = define_typ_rotation(sol['STREET'][ij])
    ...
    for ij in np.ndindex(sol['BUILDING'].shape[:2]):
        if grid['BUILDING'][ij] == 1:
            new_piece = Piece('b', piecesize)
            new_piece.update(((ij[1] - 1) * piecesize + (piecesize / 2), (ij[0] - 1) * piecesize + 50 + (piecesize / 2)))
            col.append((new_piece.surf, new_piece.rect))
    return col
